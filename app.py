        ...
        if not bulk_raw.strip():
            ...
            for block in blocks:
                ...
                st.markdown(
                    ...
                    f"Score: {score}/100 · **{status}** · {', '.join(flags)}"
                )
    ...
    with dash_tab1:
        ...
        if not filtered:
            ...
            table_rows = [{
                ...
                "County": l.get("county", ""),
                "Pipeline": l.get("pipeline_stage", "Cold"),
                "Calls": l.get("calls", 0),
            ...
            lead_labels = {
                l["id"]: f"{l.get('decedent', 'Unknown')} — {l.get('pipeline_stage', 'Cold')}"
                for l in filtered
            }
            ...
            if lead:
                ...
                with e1:
                    new_stage = st.selectbox(
                        "Pipeline Stage",
                        PIPELINE_STAGES,
                        index=PIPELINE_STAGES.index(lead.get("pipeline_stage", "Cold")),
                ...
                if lead.get("notes"):
                    ...
                    for note in lead["notes"][:8]:
                        ts = note.get("ts", "")[:10]
                        st.markdown(f"- *{ts}* · **{note.get('by', '')}:** {note.get('text', '')}")
