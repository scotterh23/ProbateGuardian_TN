...
st.markdown(
    ...
    .agent-hero {
        background: linear-gradient(135deg, #0d2818 0%, #238636 50%, #1f6feb 100%);
        color: #f0f6fc;
        ...
        line-height: 1.5;
        box-shadow: 0 6px 24px rgba(35, 134, 54, 0.28);
    }
    .agent-links {
        background: #161b22;
        border: 1px solid #30363d;
...
AGENT_HARVEST_LINKS = {
    ...
    "Sumner County Assessor": MIDDLE_TN_COUNTY_LINKS["Sumner County"]["assessor"],
...
def agent_generate_bv_search_string(contact: str, address: str, county: str, block: str = "") -> str:
    ...
    fn, ln = _parse_name_parts(contact_clean)
...
def agent_parse_notice_block(block: str, default_county: str = "Sumner County") -> dict:
    parsed = parse_court_row_block(block, default_county=default_county)
    obit_m = AGENT_OBIT_NAME_RE.search(block)
    if obit_m and parsed.get("decedent") in ("Unknown Decedent", ""):
        name = _clean_person_name(obit_m.group(1))
        if _is_valid_decedent(name):
    ...
    if not pr_heir or pr_heir == "Contact TBD":
        ...
        if pr_m:
            pr_heir = _clean_person_name(pr_m.group(1))
        else:
    ...
    if not date_hint:
        ...
        elif DATE_RE.search(block):
            date_hint = DATE_RE.search(block).group(0)
    address = parsed.get("address", "Address TBD")
...
def agent_parse_extract(raw: str, county: str = "Sumner County") -> list:
    ...
    if not blocks:
        blocks = split_court_export(raw, default_county=county)
        results = []
        for block in blocks:
            if not block.strip():
                continue
            parsed = parse_court_row_block(block, county)
            results.append({
                "decedent": parsed.get("decedent", "Unknown Decedent"),
...
def agent_push_all_high_score(results: list) -> int:
    ...
    if pushed:
        commit_leads_and_reload()
        st.session_state.branton_queue_flash = (
            f"🔥 **{pushed}** high-score newspaper leads pushed to {PARTNER_NAME}'s HOT queue"
        )
    return pushed
...
if "agent_harvest_results" not in st.session_state:
    st.session_state.agent_harvest_results = []
if "agent_harvest_raw" not in st.session_state:
    st.session_state.agent_harvest_raw = ""
if "agent_harvest_json" not in st.session_state:
    st.session_state.agent_harvest_json = ""
...
def render_ai_agent_lead_harvester(leads: list) -> None:
    """Autonomous weekly pipeline for external AI agents — paste, parse, score, queue."""
    run_serial = st.session_state.get("_run_serial", 0)
    if st.session_state.get("_agent_done_serial") == run_serial:
        return

    st.markdown(
        '<div class="agent-hero">🤖 AI Agent Lead Harvester · Small Counties<br>'
        '<span style="font-size:0.9rem;font-weight:500;">Paste raw notices → one button → '
        f'parsed leads + RE scores + BV strings → {PARTNER_NAME}\'s HOT queue</span></div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="agent-paste-label">Paste raw text from tnpublicnotice.com, Gallatin News, '
        'or any obituary/probate notice here</div>',
        unsafe_allow_html=True,
    )
    raw = st.text_area(
        "Agent harvest paste",
        ...
        label_visibility="collapsed",
        placeholder=(
            "Paste unlimited raw text — public notices, obituaries, probate filings, newspaper clips…\n\n"
            "Works with tnpublicnotice.com exports, Gallatin News clips, Hendersonville Standard obits, "
            "and messy multi-notice batches."
        ),
    )
    county = st.selectbox(
        "Default county",
        list(MIDDLE_TN_COUNTY_LINKS.keys()),
        index=list(MIDDLE_TN_COUNTY_LINKS.keys()).index("Sumner County"),
    ...
    if st.button("🚀 Run Full Agent Pipeline", use_container_width=True, type="primary", key="agent_full_pipeline"):
        if not raw.strip():
            st.warning("Paste raw notice text first.")
        else:
            ...
            st.success(
                f"✅ Pipeline complete — **{len(st.session_state.agent_harvest_results)}** records · "
                f"**{high_n}** High RE · **{queued_n}** pushed to Branton queue"
            )
            st.rerun()
    ...
    with s1:
        ...
        if st.button("1. Parse & Extract", use_container_width=True, key="agent_step_parse"):
            if not raw.strip():
                ...
                st.success(f"✅ Extracted **{len(st.session_state.agent_harvest_results)}** records")
                st.rerun()
    with s2:
        st.markdown('<div class="agent-step-marker"></div>', unsafe_allow_html=True)
        if st.button("2. Detect Real Estate Likelihood", use_container_width=True, key="agent_step_re"):
            ...
            if not results:
                ...
                st.success("✅ Real estate likelihood scored")
                st.rerun()
    with s3:
        st.markdown('<div class="agent-step-marker"></div>', unsafe_allow_html=True)
        if st.button("3. Generate Perfect BeenVerified Search Strings", use_container_width=True, key="agent_step_bv"):
            results = st.session_state.get("agent_harvest_results", [])
            if not results:
                st.warning("Run **Parse & Extract** first.")
            else:
                ...
                st.success("✅ BeenVerified search strings ready")
                st.rerun()
    with s4:
        st.markdown('<div class="agent-step-marker"></div>', unsafe_allow_html=True)
        if st.button("4. Push Qualified Lead to Branton Queue 🔥", use_container_width=True, key="agent_step_push"):
            results = st.session_state.get("agent_harvest_results", [])
            if not results:
                st.warning("Run the pipeline through step 3 first.")
            else:
                ...
                if n:
                    st.success(f"🔥 **{n}** high-score leads pushed to Branton queue")
                else:
                    st.info("No new High-score leads to push (already queued or none marked High).")
                st.rerun()
    ...
    if results:
        ...
        high_n = sum(1 for r in results if r.get("re_score_label") == "High")
        st.caption(
            f"**{len(results)}** records · **{high_n}** High RE · "
            "High-score rows auto-queue with note: *Newspaper Scrape • High Potential Asset*"
        )

        j1, j2 = st.columns(2)
        with j1:
            if st.button("📋 Copy JSON for Agent", use_container_width=True, key="agent_copy_json"):
                st.session_state.agent_harvest_json = json.dumps(
                    agent_results_for_export(results), indent=2
                )
        with j2:
            if st.button("✅ Add All High-Score Leads to Branton Queue", use_container_width=True, key="agent_push_all_high"):
                n = agent_push_all_high_score(results)
                st.session_state.agent_harvest_results = results
                st.session_state.agent_harvest_json = json.dumps(
                    agent_results_for_export(results), indent=2
                ...
                if n:
                    st.success(f"🔥 **{n}** leads added to Branton queue")
                    st.rerun()
                else:
        ...
        if st.session_state.get("agent_harvest_json"):
            st.text_area(
                "JSON export (select all → copy for Hermes / Open Claw / Claude)",
            ...
            st.download_button(
                "⬇️ Download JSON",
                data=st.session_state.agent_harvest_json,
                file_name=f"agent-harvest-{datetime.now().strftime('%Y%m%d')}.json",
                mime="application/json",
                use_container_width=True,
                key="agent_json_download",
            )

    flash = st.session_state.get("branton_queue_flash")
    if flash and results:
        st.success(flash)
